...
class Mcla():
  ...
  def convert_schedule_html(self, html, team):
    ...
    for row in rows:
      ...
      opp_id = self._parse_team_link_into_id(team['sport'], team['source'],
                                             opp_link)
      if opp_id:
  ...
  def convert_game_details_html(self, html, location, game_id, sport, source,
    ...
    if away_header:
      ...
      result['away_stats'] = self._parse_stats_tables(away_tables, sport,
                                                      source)

    home_header = soup.find('header', attrs={'title': 'Home Team'})
    if home_header:
      home_tables = home_header.find_next_siblings('table')
      result['home_stats'] = self._parse_stats_tables(home_tables, sport,
                                                      source)
      ...
      for raw_row in table.tbody.find_all('tr'):
        ...
        for i, cell in enumerate(raw_row.find_all('td')):
          ...
          if col_name == 'Field Player' or col_name == 'Goalie':
            row['player'] = {
                'id':
                self._parse_player_link_into_id(sport, source, cell.a['href']),
                'name':
                cell.a.string,
                'external_link':
                cell.a['href']
          ...
          if col_name == 'FO':
            ...
            if won or lost:
              row['face_offs'] = {'won': won or 0, 'lost': lost or 0}
          if col_name == 'GB':
