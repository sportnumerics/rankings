...
def add_parsers(parsers):
  ...
  all_parser.add_argument('--source', choices=all.ALL_SOURCES, action='append', default=[], help='add a source (mcla or ncaa) - or specify --all-sources to scrape')
  all_parser.add_argument('--all-sources', action='store_true', help='Scrape all available sources')
